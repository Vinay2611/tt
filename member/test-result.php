...
if($result->num_rows>0){
    ...
    $TestID=$PageData['id'];
    $TestDuration=$PageData['test_duration'];
...
<div class="row-diff">
    <?php if(isset($error) && $error){
        ...
        $new_credits=intval($credits_remaining)-1;
        $sql2 = "update member set credits_remaining='$new_credits' where email='$member_email'";
        $result2 = $conn->query($sql2);
        $test_date=date('Y-m-d H:i:s');

        $sql4 = "insert into creditflow (member_id,`date`,credits,narration) values ('$member_id','$test_date','-1','Certification test taken')";
        if($conn->query($sql4)){
            if($conn->insert_id){
                $insert_id=$conn->insert_id;
                $sql3 = "insert into results (member_id,test_date,transaction_id,test_duration,test_time,UWPM,UCPM,accuracy,avg_CWPM) values ('$member_id','$test_date','$insert_id','$TestDuration','$TestTime','$WPMUncorrected','$CPMUncorrected','$Accuracy','$WPMCorrect')";
               if($conn->query($sql3)){
    ...
    <div class="col-md-12 no-padding">
        ...
        <div class="col-md-6">
            <img src="<?php echo $base_url;?>images/smallcertificate.jpg">
        </div>
    </div>
    ...
    <div align="center">
        <p>
            Now you can print your certificate online or you can order a printed certificate from us.
        </p>
        <a href="<?php echo $base_url;?>member/print_certificate.php?id=<?php echo isset($result_id)?$result_id:'';?>" class="btn btn-default">Print Certificate Online</a>
        <a href="<?php echo $base_url;?>member/printed-certificates.php?id=<?php echo isset($result_id)?$result_id:'';?>" class="btn btn-default">Order Printed Certificate</a>
        <br>
        <br>
        <p>
            Practice again, take a certification test or tell your friends about this site.
        </p>
        <a href="<?php echo $base_url;?>member/start-test.php" class="btn btn-default">Take Another Certification Test</a>
        <a href="<?php echo $base_url;?>tell-friend.php"class="btn btn-default">Tell A Friend</a>
