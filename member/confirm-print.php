...
$certificate_remaining=$userData['certificates_remaining'];
...
if(empty($certificate_remaining) || $certificate_remaining==0 || $certificate_remaining=='' || $certificate_remaining<1){
    $msg="<br><p>Your Certificate Credit is 0! please Purchase Credits</p>";
    $error=true;
}else {
    $cert = intval($certificate_remaining) - 1;
    $sql = "update member set certificates_remaining='$cert' where email='$member_email'";
    $result = $conn->query($sql);

    $sql11 = "insert into orders (member_id,result_id) values ('$member_id','$rid')";
